...
function responsive_settings( $block_content, $block, $uuid = false ) {
	...
	if ( ! isset( $block['attrs']['GozaResponsiveSettings'] ) || empty( $block['attrs']['GozaResponsiveSettings'] ) ) {
		return $block_content;
	}

	// Generate a unique id to use as a class name so we can target it with CSS.
	$unique_class_name = 'block-' . ( $uuid ? $uuid : uniqid() );
	...
	uksort(
		$block['attrs']['GozaResponsiveSettings'],
	...
	foreach ( $block['attrs']['GozaResponsiveSettings'] as $breakpoint_width => $responsive_settings ) {
